...
void Setup(){
  ...
  int nr_player;
  std::cout << " [setup]  number of players: ";
  std::cin >> nr_player;
  std::cout << std::endl;
  ...
  for(unsigned int i=0; i<nr_player; ++i){
    ...
    std::cout << " [setup]  name player "<< i+1 <<": ";
    std::cin >> name_temp;
    std::cout << std::endl;
  ...
  // setup start score
  std::cout << " [setup]  Please set your start score: ";
  std::cin >> start;
  std::cout << std::endl;
...
void ThrowDarts(player *p){
  ...
  for(int dartnr=1; dartnr<=3; dartnr++){
    ...
    while(points[dartnr-1] > 60){
      std::cout << " [error]  you can not score more than 60 points, try again: ";
      std::cin >> points[dartnr-1];
    }
    ...
    if(points[dartnr-1] > p->ScoreLeft){
      std::cout << " [info]  you can not end below 0, score is reset to: " << oldscore << std::endl;
...
bool PlayAgain(){
  std::string rematch;
  std::cout << " [info]  do you want to play another leg? (y/n): ";
  std::cin >> rematch;
...
void EndGame(){
  ...
  while(true){
    std::cout << std::endl << std::endl << std::endl;
    std::cout << " [info]  Type 'q' to quit (and save data): ";
    std::cin >> quit;
    if(quit == "q") return;
  }
}
